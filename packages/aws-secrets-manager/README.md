# Spark⚡️Bot - AWS Secrets Manager secrets vault plugin
...
@sparkbot/plugin-secrets-aws implements AWS Secrets Management storage for [Spark⚡️Bot](https://github.com/SparkBotDev/SparkBot) secrets management.
...
1. Install the plugin `bun add @sparkbot/plugin-secrets-aws`
2. Update `sparkbot.config.ts`:

```ts
secretsVaultPlugin: {
	prod: {
		module: '@sparkbot/plugin-secrets-aws',
